...
requests.forEach(  (req) => {
    productcontainer.innerHTML += `
    <div class="container">
    <h1>المبلغ:${req.price}</h1>
    <p>الرقم:${req.number}</p>
    <p>طريقة الدفع:${req.select}</p>
    <P>الحالة:${req.status}</p>
    <p>الميلغ بعد العمولة:${req.discount}</p>
    <button onclick = "deletebtn(${req.id })">حذف</button>
    </div>
...
async function deletebtn(id) {
    let saved = JSON.parse(localStorage.getItem('myrequests_ids')) || []
    let item = saved.find(i => i.id === id)
    if (!item) return
const {error} = await _supabase.from('requests').delete().eq('id', id).eq('secret_code' , item.secret_code)
if (error){
    console.error('حدث خطا اثناء الحذف')
    return
} 
